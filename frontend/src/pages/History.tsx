...
export function HistoryPage() {
  ...
  return (
    <>
      ...
      <section className="panel"><div className="panel__header"><div><p className="eyebrow">LIVE ALERT AUDIT TRAIL</p><h2>Recent activity</h2></div></div>
        <QueryState state={activity} empty={(data) => data.items.length === 0}>
          {(data) => <div className="table-scroll"><table className="alerts-table"><thead><tr><th>Time</th><th>Alert</th><th>Actor</th><th>Action</th><th>Details</th><th /></tr></thead><tbody>
            {grouped.map((entry, index) => <tr key={`${entry.alert_id}-${entry.timestamp}-${index}`}><td>{formatDate(entry.timestamp)}</td><td className="mono">{entry.alert_id}</td><td>{entry.actor}</td><td><StatusBadge value={entry.action} /></td><td>{JSON.stringify(entry.details ?? {})}</td><td><Link className="table-link" to={`/alerts/${encodeURIComponent(entry.alert_id)}`}>Inspect →</Link></td></tr>)}
          </tbody></table></div>}
        </QueryState>
      </section>
    </>
