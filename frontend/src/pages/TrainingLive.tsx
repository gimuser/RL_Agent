...
const fmt = (v: number | null | undefined, d = 4) => typeof v === "number" && Number.isFinite(v) ? v.toFixed(d) : "—";
...
function ActionsChart({ history, windowSize }: { history: AuthoritativeHistoryPoint[]; windowSize: number }) {
  ...
  const series = Object.keys(colors).map((action) => ({ action, points: visible.map((p, i) => { const c = Number((p.action_distribution as any)?.[action] ?? 0); const total = Object.values(p.action_distribution ?? {}).reduce<number>((s, v) => s + Number(v || 0), 0) || 1; return { x: left + i/Math.max(1,visible.length-1)*pw, y: top + (1-c/total)*ph, e:p.epoch, ratio:c/total }; }) }));
