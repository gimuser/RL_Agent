...
const metric = (p: AuthoritativeHistoryPoint, k: Key) => Number((p as any)[k] ?? (k === "policy_reward" ? p.avg_reward : 0) ?? 0);
const fmt = (v: number, d = 4) => Number.isFinite(v) ? v.toFixed(d) : "—";

function Chart({ title, history, field, color, windowSize, percent = false }: { title: string; history: AuthoritativeHistoryPoint[]; field: Key; color: string; windowSize: number; percent?: boolean }) {
  ...
  const values = visible.map(p => metric(p, field)); const min0 = Math.min(...values), max0 = Math.max(...values); const pad = (max0 - min0 || Math.max(Math.abs(max0) * 0.08, 1)) * 0.08; const min = min0 - pad, max = max0 + pad; const span = max - min || 1;
  const pts = visible.map((p, i) => ({ x: left + i / Math.max(1, visible.length - 1) * pw, y: top + (1 - (metric(p, field) - min) / span) * ph, e: p.epoch, v: metric(p, field) }));
  const line = pts.map(p => `${p.x},${p.y}`).join(" ");
  return <div className="chart-card"><div className="chart-title"><span>{title}</span><strong>Epoch {visible.at(-1)?.epoch}</strong></div><svg viewBox={`0 0 ${width} ${height}`} className="telemetry-svg" role="img" aria-label={title}>
    {[0, .25, .5, .75, 1].map(r => { const y = top + r * ph; const val = max - r * span; return <g key={r}><line x1={left} y1={y} x2={left + pw} y2={y} className="grid-line"/><text x={left - 10} y={y + 4} textAnchor="end" className="axis-label">{percent ? `${(val * 100).toFixed(0)}%` : field === "total_updates" ? nf.format(Math.round(val)) : val.toFixed(3)}</text></g>; })}
    <polyline points={line} fill="none" stroke={color} strokeWidth="4" strokeLinejoin="round" strokeLinecap="round"/>
    {pts.map(p => <circle key={p.e} cx={p.x} cy={p.y} r="4.5" fill={color}><title>{`Epoch ${p.e} · ${percent ? (p.v*100).toFixed(2)+"%" : p.v.toFixed(6)}`}</title></circle>)}
    <text x={left} y={height - 12} className="axis-label">Epoch {visible[0].epoch}</text><text x={left + pw} y={height - 12} textAnchor="end" className="axis-label">Epoch {visible.at(-1)?.epoch}</text>
  </svg><div className="chart-meta"><span>{visible.length} epochs</span><span>min {fmt(min0, 5)}</span><span>max {fmt(max0, 5)}</span></div></div>;
}

function ActionsChart({ history, windowSize }: { history: AuthoritativeHistoryPoint[]; windowSize: number }) {
  const visible = history.slice(-Math.max(2, Math.min(windowSize, history.length))); if (!visible.length) return <div className="chart-empty">Waiting for action telemetry…</div>;
  ...
  const series = Object.keys(colors).map((action) => ({ action, points: visible.map((p, i) => { const c = (p.action_distribution as any)?.[action] ?? 0; const total = Object.values(p.action_distribution ?? {}).reduce((s, v) => s + Number(v || 0), 0) || 1; return { x: left + i/Math.max(1,visible.length-1)*pw, y: top + (1-c/total)*ph, e:p.epoch, ratio:c/total }; }) }));
  return <div className="chart-card"><div className="chart-title"><span>Action distribution</span><strong>Dynamic by epoch</strong></div><svg viewBox={`0 0 ${width} ${height}`} className="telemetry-svg">
  ...
  </svg><div className="legend"><span><i style={{background:colors.allow}}/>Allow</span><span><i style={{background:colors.block}}/>Block</span><span><i style={{background:colors.human_review}}/>Human review</span></div></div>;
}
...
export function TrainingLive() {
  ...
  return <div className="live-page">
    <header className="live-hero"><div><div className="live-kicker">RL CONTROL ROOM · LIVE TRAINING</div><h1>{t?.display_name ?? "Adaptive experiment"}</h1><p>Live learning telemetry, convergence monitoring, resource-safe execution, and candidate evaluation.</p></div><div className="live-actions"><button className="live-btn" onClick={()=>navigate("/training")}>Choose models</button><button className="live-btn" onClick={()=>void refresh()}>Refresh</button>{state?.status==="running"&&<button className="live-btn live-btn--danger" disabled={stopping} onClick={()=>void stop()}>{stopping?"Stopping…":"Stop training"}</button>}</div></header>
    {error&&<div className="live-error">{error}</div>}
    <section className="live-kpis"><div><span>Status</span><strong className={`status ${statusClass}`}>{state?.status??"idle"}</strong></div><div><span>Model</span><strong>{t?.display_name??"—"}</strong></div><div><span>Epoch</span><strong>{nf.format(t?.actual_epochs??latest?.epoch??0)} / {nf.format(t?.epochs??0)}</strong></div><div><span>Updates</span><strong>{nf.format(t?.total_updates_used??0)}</strong></div><div><span>Best epoch</span><strong>{nf.format(t?.best_epoch??0)}</strong></div><div><span>Stop reason</span><strong>{t?.stopping_reason??"learning"}</strong></div></section>
    ...
    <section className="live-bottom-grid"><article><div className="panel-title">Selected models</div><div className="chips">{selected.length?selected.map(s=><span key={s}>{s}</span>):<span>—</span>}</div><div className="panel-title">Champion</div><strong>{best?.name??"Not selected until comparison completes"}</strong></article><article><div className="panel-title">Live 40-alert evaluation</div><div className="live-stats"><span>Considered<strong>{live?.alerts_considered??"—"}</strong></span><span>Processed<strong>{live?.alerts_processed??"—"}</strong></span><span>Human review<strong>{live?.human_review_routed??"—"}</strong></span></div></article></section>
  </div>;
}
