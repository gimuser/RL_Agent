...
class IQLProxy(DoubleDQN):
    """Discrete IQL with explicit reward-derived behavior proxy."""
    ...
    @torch.no_grad()
    def act(self, states):
        logits = self.policy(torch.as_tensor(states, dtype=torch.float32, device=self.device))
...
class BCQProxy(DoubleDQN):
    """Discrete BCQ with explicit reward-derived behavior proxy."""
    ...
    def save(self, path: str):
        torch.save({"algorithm": "bcq", "online": self.online.state_dict(), "target": self.target.state_dict(), "behavior": self.behavior.state_dict(), "gamma": self.gamma, "n_actions": self.n_actions}, path)
...
def build_model(name: str, *, input_dim: int, n_actions: int, learning_rate: float, gamma: float, hidden_dim: int = 128):
    ...
    common = dict(input_dim=input_dim, n_actions=n_actions, learning_rate=learning_rate, gamma=gamma, hidden_dim=hidden_dim)
